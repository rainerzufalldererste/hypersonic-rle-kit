...
int main(int argc, char **pArgv)
{
  ...
  // Validate Parameters.
  {
    ...
    if (mmtfMode && extremeSize != 128 && extremeSize != 256)
    {
      puts("MMTF Mode only supports mtf width of 128 or 256.");
      return 1;
    }

    benchmarkAll = !normalMode && !ultraMode && !extremeMode && !mmtfMode;
  }
